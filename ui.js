...
const ICONS = {
  ...
  grid: '<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>',
  shield: '<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l8 4v6c0 5-3.4 8.4-8 10-4.6-1.6-8-5-8-10V6z"/></svg>',
  ...
  receipt: '<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2h16v20l-3-2-3 2-3-2-3 2-3-2-1 1z"/><line x1="8" y1="7" x2="16" y2="7"/><line x1="8" y1="11" x2="16" y2="11"/></svg>',
  ...
  chat: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="18" height="18"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/></svg>',
  ...
  smartphone: '<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="5" y="2" width="14" height="20" rx="2"/><line x1="12" y1="18" x2="12.01" y2="18"/></svg>',
  database: '<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><ellipse cx="12" cy="5" rx="8" ry="3"/><path d="M4 5v7c0 1.7 3.6 3 8 3s8-1.3 8-3V5"/><path d="M4 12v7c0 1.7 3.6 3 8 3s8-1.3 8-3v-7"/></svg>',
  sim: '<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2h8l4 4v16H6z"/><path d="M10 2v5h6"/><rect x="9" y="11" width="6" height="6" rx="1"/><path d="M10 19h4"/></svg>',
  gamepad: '<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6.5 8h11a4.5 4.5 0 0 1 4.3 5.8l-1.1 3.5a2.5 2.5 0 0 1-4.6.4l-1.2-2.2H9.1l-1.2 2.2a2.5 2.5 0 0 1-4.6-.4l-1.1-3.5A4.5 4.5 0 0 1 6.5 8z"/><path d="M7 11v4M5 13h4M16 12h.01M19 14h.01"/></svg>',
...
/* ---- Toasts ---- */
const TOAST_ICON = { success: 'checkCircle', error: 'alertCircle', warning: 'alertTriangle', info: 'info' };
function showToast({ type = 'info', title, message, duration = 4200 }){
  ...
  el.innerHTML = `
    <span class="toast__icon">${icon(TOAST_ICON[type])}</span>
    <div>
      <div class="toast__title">${escapeHTML(title)}</div>
      ${message ? `<div class="toast__msg">${escapeHTML(message)}</div>` : ''}
    </div>
    <button class="toast__close" aria-label="${t('close')}">${icon('x')}</button>
  `;
...
function openModal({ title, bodyHTML, footerHTML, onMount, size }){
  ...
  backdrop.innerHTML = `
    <div class="modal" role="dialog" aria-modal="true" aria-labelledby="modalTitle" ${size ? `style="max-width:${size}"` : ''}>
      <div class="modal__header">
        <h3 id="modalTitle">${escapeHTML(title)}</h3>
        <button class="modal__close" aria-label="${t('close')}">${icon('x')}</button>
      </div>
...
function closeModal(){
  const el = qs('#activeModal');
  if (el){ el.remove(); document.body.style.overflow = ''; }
  document.removeEventListener('keydown', escCloseModal);
}

function openConfirm({ title, message, confirmLabel, danger = true, onConfirm }){
  const backdrop = openModal({
    title,
    bodyHTML: `<p class="mb-0">${escapeHTML(message)}</p>`,
    footerHTML: `
      <button class="btn btn-secondary" data-act="cancel">${t('cancel')}</button>
      <button class="btn ${danger ? 'btn-danger' : 'btn-primary'}" data-act="confirm">${escapeHTML(confirmLabel || t('confirm'))}</button>
    `,
  });
...
function skeletonLines(n = 3){
  return Array.from({ length: n }).map(() => `<div class="skeleton skel-line" style="width:${60 + Math.random()*35}%"></div>`).join('');
}
function skeletonCards(n = 3){
  return `<div class="grid grid-3">${Array.from({ length: n }).map(() => `<div class="skeleton skel-card"></div>`).join('')}</div>`;
}

/* ---- Empty / error state blocks ---- */
function emptyState({ title, desc, iconName = 'inbox' }){
  return `<div class="state-block"><div class="state-icon">${icon(iconName)}</div><h3>${escapeHTML(title)}</h3><p>${escapeHTML(desc)}</p></div>`;
}
function errorState({ onRetryAttr }){
  return `<div class="state-block"><div class="state-icon">${icon('alertTriangle')}</div><h3>${t('error_title')}</h3><p>${t('error_desc')}</p><button class="btn btn-outline" ${onRetryAttr}>${icon('refresh')} ${t('retry')}</button></div>`;
}
...
function setupDropdowns(){
  qsa('.dropdown-trigger').forEach(trigger => {
