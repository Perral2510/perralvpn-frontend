...
PAGES['#/payment-result'] = async (root) => {
  ...
  const description = status === 'success'
    ...
    : status === 'cancel'
      ? 'Thanh toán đã được hủy và đơn hàng đã chuyển sang trạng thái Đã hủy.'
  ...
  if (status === 'cancel') {
    const cancelled = await RealAPI.cancelOrder(orderId);
    if (!cancelled.ok && cancelled.status !== 400) {
      statusText.textContent = cancelled.error || 'Không thể cập nhật trạng thái hủy đơn.';
      return;
    }
  }
  let attempts = 0;
